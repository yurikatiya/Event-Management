    ...
    <div class="mx-auto max-w-[1400px]">
        <header class="mb-8 flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-base text-slate-400">{{ $sponsors->total() }} sponsor terdaftar</p>
            </div>
            <a href="{{ route('admin.sponsors.create') }}" class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-2xl font-light leading-none text-slate-700 shadow-[0_8px_18px_rgba(15,23,42,0.08)] transition duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_12px_22px_rgba(15,23,42,0.12)]" style="line-height: 1;" title="Tambah Sponsor" aria-label="Tambah Sponsor">+</a>
        </header>

        <form method="GET" class="mb-6 max-w-sm">
            <div class="relative">
                <i class="bi bi-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                <input name="search" value="{{ request('search') }}" placeholder="Search sponsors..." class="h-11 w-full rounded-xl border border-slate-200 bg-white pl-11 pr-4 text-sm outline-none focus:border-sky-400">
            </div>
        </form>
            ...
            @forelse ($sponsors as $sponsor)
                <article class="rounded-2xl border border-[#dceaf7] bg-white px-5 py-6 text-center shadow-sm">
                    ...
                    <div class="mt-5 flex items-center justify-center gap-3">
                        <a href="{{ route('admin.sponsors.edit', $sponsor) }}" title="Edit sponsor" class="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-white text-lg font-light leading-none text-slate-700 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_8px_14px_rgba(15,23,42,0.08)]" style="line-height: 1;">+</a>
                        <form method="POST" action="{{ route('admin.sponsors.destroy', $sponsor) }}" onsubmit="return confirm('Hapus sponsor ini?')">@csrf @method('DELETE')<button title="Delete sponsor" class="flex h-9 w-9 items-center justify-center rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-100"><i class="bi bi-trash3"></i></button></form>
                    </div>
