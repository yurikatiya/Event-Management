        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <form method="POST" action="{{ route('admin.gallery.store') }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="title" name="title" value="{{ old('title') }}" required class="form-input h-11 rounded-xl border-slate-200 text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-400">
                    @error('title')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="form-input rounded-xl border-slate-200 text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-400">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="image" type="file" name="image" accept="image/*" required class="form-input rounded-xl border-slate-200 p-2 text-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-400">
                    @error('image')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
